...
class GetAllItems(Resource):
    def get(self):
        try: 
            ...
            parser.add_argument('dayID',type =int)
            args = parser.parse_args()
            _userID = args['userID']
            _dayID = args['dayID']
            conn = mysql.connect()
            cursor = conn.cursor()
            cursor.callproc('sp_GetAllItems',(_userID,_dayID))
            data = cursor.fetchall()
...
class AddItem(Resource):
    def post(self):
        try: 
            # Parse the arguments
            parser = reqparse.RequestParser()
            parser.add_argument('routineID', type=int)
            parser.add_argument('exerciseName', type=str)
            parser.add_argument('dayID',type=int)
            parser.add_argument('exerciseType',type=int)
            args = parser.parse_args()

            _routineID = args['routineID']
            _exerciseName = args['exerciseName']
            _dayID = args['dayID']
            _exerciseType = args['exerciseType']


            conn = mysql.connect()
            cursor = conn.cursor()
            cursor.callproc('sp_AddItems',( _routineID, _exerciseName, _dayID, _exerciseType))
            data = cursor.fetchall()
